<template>
  <div class="d-flex flex-column min-vh-100">
    ...
    <div class="position-relative flex-fill">
      <!-- background color -->
      <div
        class="background position-absolute w-100 z-n1"
        :style="{ backgroundColor: backgroundColor }"
        style="height: 225px"
      ></div>
      <!-- content -->
      <div class="m-4 p-4 d-flex justify-content-center">
        <div class="bg-white rounded-3 shadow w-75">
...
const backgroundColor = computed(() =>
  authStore.user.role === "student" ? "#FFEEE8" : "#FFC7B2"
);

const placeholderImageUrl = computed(() => {
  const letters = authStore.user.name
    ? getFirstLetter(authStore.user.name)
    : "";
